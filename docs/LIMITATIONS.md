...
- **Barge-in is NOT working yet (experimental).** The caller *delivers* a second utterance while the agent is replying, but — verified with Soundcheck's own oracle (STT on the full conversation audio) — **the agent is not actually interrupted**: it completes its entire answer (e.g. lists all three specials) and only then addresses the second question. The Deepgram VA, given our buffered (non-real-time) audio sending, treats the interrupt as a queued follow-up rather than a barge-in. Faithful barge-in needs **real-time duplex**: pace the agent's audio playback in real time, send the interrupt mid-playback so the VA's server-side barge-in fires, and **mix** (overlay) the two tracks for the report (concatenation can't represent simultaneous speech). That is future work and shares its foundation with a live `--listen` mode. Until then, treat the barge-in scenario as a delivery demo, not a validated capability.
