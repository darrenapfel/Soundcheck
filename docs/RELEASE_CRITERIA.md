...
- [x] **Release tag honesty (P1).** The README Action snippet pins `@v2.0.0-rc.1` (an existing tag) rather than the not-yet-cut `@v2`. **Release step:** cut `v2.0.0` / move `@v2` to the release commit before the snippet is presented as the stable copy-paste.
- [ ] **Clean-tree release guard (P2).** Release **only from a clean working tree** — `npm pack` includes all of `docs/`, so local doc edits leak into the artifact. Pre-publish check: `git diff --quiet && git diff --cached --quiet` must pass on the release commit.
