...
- [ ] **Installed-npm-package execution (P0 — open product decision).** Node refuses to strip TypeScript types under `node_modules`, so `npm install soundcheck` + run the CLI currently fails (`ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING`); `main`/`types` also point at `.ts` source. The repo-clone and GitHub-Action paths (how the artifact is actually exercised) are unaffected. Resolving this is an architecture decision — either (a) add a publish-time build emitting `dist/**/*.js` + `.d.ts` and point `bin`/`main`/`types`/`exports` at `dist` (keeps zero *runtime* deps; preserves the no-build *dev* workflow), or (b) scope distribution to repo-clone + Action and drop the npm-registry-install claim. **Pending owner decision.**
