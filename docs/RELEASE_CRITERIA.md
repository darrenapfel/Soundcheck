...
- [x] **Self-improving loop shipped + closed offline.** `run --promote-failures` (`src/regress/promoteTrace`) freezes a discovered failing call into a scripted, replayable regression carrying the same invariants; `examples/self-improving-loop/` runs the full discover → promote → refine flywheel (oracle-validated live); the closure is pinned offline in `test/regress.test.ts` (reproduces-on-broken / green-on-fixed). Reviewed by an independent sub-agent.
- [x] **Nine gates in `src/gates/index.ts`** (vs. the v1.0 "6 gates" line above): `no_spoken_symbols`, `no_spoken_cardinal_ids`, `required_tool`, `forbidden_tool`, `tool_sequence`, `tool_args_match_schema`, `spoken_matches_tool`, `grounding`, `latency` — a composable registry; `test/gates.test.ts` + `test/self-test.test.ts` exercise them.
- [x] **`npm run validate` green + deterministic; fresh keyless clone validates; CI offline.** Current suite **148 deterministic tests, 0 lint errors and 0 warnings** (`eslint . --max-warnings=0` is clean as of the round-2 readiness + caller Phases 1–3 + round-3/4 fixes — the former `no-explicit-any` warnings in `src/deepgram.ts` + `test/adapter.test.ts` are resolved); verified on a true `/tmp` keyless clone and across repeated runs (supersedes the v1.0 "65/65" figure above).
...
- [x] **Every STS-v2 milestone independently reviewed; final multi-agent panel signed off.** `REVIEW_LOG.md` has rows for STS-v2 M1–M8, the **final 3-agent release panel** (security+correctness → GO, docs+DX → GO, done-criteria → 4/6 with all findings addressed), and the subsequent **self-improving-loop** review (independent opus sub-agent, ship-with-fixes → fixed). The RC tag was re-cut on the reviewed HEAD.
- [x] **`README_ASPIRATIONAL.md` promoted to `README.md`** because every promise is literally true or de-scoped in writing.

## Public-release readiness (round-2 review — `docs/SOUNDCHECK_ROUND2_READINESS_REVIEW.md`)

> The round-two review (score 78/100) confirmed the repo-clone demo and CI-Action paths are solid and re-scored the remaining gaps for a *public package* release. Status of each finding:

- [x] **Machine-readable example contract (P1).** Every scenario under `examples/**` + `scenarios/**` is now exactly one of: replay-backed (a cassette exists), `liveOnly` (goal-driven), or `fixtureOnly` (authoring/tuning input). `test/example-contract.test.ts` enumerates the full set and fails on any hole or conflicting marker; `run --replay` skips `liveOnly`/`fixtureOnly` with an honest message instead of erroring on a missing cassette.
- [x] **Windows cassette-path containment (P1).** `isWithinRoot` uses `path.relative` (separator-agnostic) rather than `startsWith(root + "/")`; covered for both POSIX and Windows separators in `test/cassette.test.ts`.
- [x] **Strict lint clean (P2).** `eslint . --max-warnings=0` passes; the STT JSON parse has a narrow `DeepgramListenResponse` type and the adapter test drops its `as any`.
- [x] **README/example polish (P2).** Stray fence removed from `examples/interactive/README.md`; `examples/support/README.md` commands are now copy-paste offline (`--replay`); the restaurant table states the `bare`/`hardened` replay caveat (`--only book-modify-confirm`).
- [x] **Release tag honesty (P1) — done.** The `v2.0.0` (immutable) and `v2` (major alias) stable tags are cut on the released HEAD; the README Action snippet pins `@v2`.
- [x] **Clean-tree release guard (P2) — done.** Release **only from a clean working tree** — `npm pack` includes all of `docs/`, so local doc edits would otherwise leak into the artifact. The `release:check` npm script enforces it (`npm run release:check` → fails unless `git status --porcelain` is empty; gitignored `dist/`/`runs/` don't count). **Release sequence:** `npm run release:check && npm run validate && npm run smoke`, then tag on the clean HEAD.
- [x] **Installed-npm-package execution (P0 — resolved, Option A).** Node refuses to strip TypeScript types under `node_modules`, which broke `npm install soundcheck` + run the CLI. Fixed with a **publish-time build**: `tsconfig.build.json` emits `dist/**/*.js` + `.d.ts` from `src/` (`.ts` import specifiers rewritten to `.js`), run by `prepack` so the published tarball ships built JS; `bin`/`main`/`types`/`exports` point at `dist`. **Development is unchanged** — the npm scripts still run raw `.ts` via `--experimental-strip-types`, so there is no build step for dev, and zero *runtime* deps still holds (`typescript` is a devDep). The `bin` keeps the strip-types flag so a consumer's own `.ts` agent (outside `node_modules`) still loads. Proven by `scripts/smoke-package.sh` (pack → install the tarball into a throwaway consumer → import the package + run an offline replay against a consumer-side `.ts` agent), wired as a separate **`smoke`** CI job.
