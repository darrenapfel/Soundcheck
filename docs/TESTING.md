...
`npm run validate` = typecheck + lint + unit + replay-integration. It is the merge gate, and it is what CI runs. It must never depend on a live call. As of the current HEAD it is **256 tests, all passing — fully offline, no key, no network.** The suite lives in `test/*.test.ts`; the inventory:

| Test file(s) | What it pins |
|---|---|
| `gates.test.ts` | each of the 11 registry gates passes the conforming case and fails the violation — including the prose-date tool-arg regression (`tool_args_match_schema` rejects `"October seventh"`), pinned deterministically here rather than relying on a cassette that may record an ISO date, and `spoken_matches_text` proven both ways (canonical equivalence passes; a real misheard time fails; an unknown 1-based turn number fails closed; a token-less expected text fails closed rather than vacuously matching a silent turn), and the red-team inversion proven both ways (attacker victory emits a failing `attack_succeeded` row; a defended call adds no synthetic row — including when the run is not goal-driven) |
...
| `downstream-api.test.ts` | the surface downstream tools read: `transcribeFile` maps a full `/v1/listen` body (transcript, confidence, word timeline, utterances, duration) and sends the right request — repeated `keyterm` parameters, `utterances`, the container content type, and NO `encoding`/`sample_rate` — against a fake fetch; a shape change degrades to empty values rather than throwing; `--offline` refuses before any request leaves the process (fetch is asserted never called); `similarity` scores 1 for formatting equivalences and folded contractions, ignores fillers only when asked; the diff and the score stay under 200 MB on 10,000-token inputs; `findPhrase` locates a line in a word timeline, honors the time window, tolerates one mishearing, and returns null when absent; `judgeText` grades bare text and refuses an empty transcript; and the `stt`/`judge` commands fail closed on a bad invocation |
...
| `adapter.test.ts`, `adapter-loop.test.ts`, `genericity.test.ts` | the adapter config surface, the duplex socket loop (including the `toolsCalled` threading that lets a `stopWhen` scenario end the call at the decisive turn), and the same scenarios running unchanged against a non-Deepgram (mock) target |
| `caller.test.ts`, `caller-policy.test.ts`, `capture.test.ts`, `normalize.test.ts`, `cassette.test.ts`, `trace.test.ts`, `judge.test.ts`, `security.test.ts`, `deepgram-key.test.ts` | caller phrasing/policy (incl. all closed caller gaps: termination reasons, silence prod, cross-persona push-back, committed facts, distinct voices, goal-driven barge-in, and the stopWhen early hang-up), capture, normalization/detection, cassette + Trace round-tripping (incl. POSIX/Windows path containment), the judge over fixture transcripts, the no-credential-leak security check, and the key-resolution short-circuit (an explicitly empty `DEEPGRAM_API_KEY` never falls through to `.env` files) |
