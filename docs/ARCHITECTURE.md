...
- **`spoken_matches_text`** — the agent's spoken words carry the same *content* as an expected sentence, under the normalization-aware compare (`src/compare/`): formatting equivalences pass, real content errors fail with a token-level diff. Optional `turn` pins a specific 0-based turn (out-of-range fails closed); without it, any turn may match.
...
├── test/                # the deterministic suite (234 tests; see TESTING.md)
