...
| `stopWhen` | `{ toolCalled }` (optional) | End the call as soon as the agent calls this tool. A scripted tape cannot react, so without it a scenario probing a destructive action keeps reading lines that plead for something already done. Ends with `terminationReason: "objective_observed"`. Needs the Caller-driven path — honored by the live Voice Agent adapter, ignored by the mock adapter (which replays a pre-baked list). Scripted callers only: a goal-driven caller (scenario `goal` / `--caller goal`) improvises its own ending and ignores `stopWhen`. |
