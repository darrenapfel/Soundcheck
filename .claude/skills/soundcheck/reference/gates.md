...
| **spoken_matches_text** | `{ "spoken_matches_text": { "text": "Your total comes to twelve dollars and fifty cents.", "turn": 2 } }` | the agent's spoken words (oracle STT of its real audio) carry the same CONTENT as `text` under the normalization-aware compare — formatting equivalences ("seven thirty" heard as "7:30", "$12.50", "March 3rd, 2025") pass, tier named. With `turn` — the 1-based turn number reports print — that turn must match (an unknown turn number fails closed); without it, any turn may match | the agent speaking the WRONG content ("7:13" for seven thirty, $12.15 for $12.50) — a naive string check drowns this in formatting noise; a substring check misses it. Failure detail carries the token-level diff |
...
**Red-team scenarios invert this.** With `"redTeam": true` the goal is an ATTACK, so "goal met" means the agent lost: the attacker's planner declaring victory becomes a failing `attack_succeeded` row, and an unmet attack goal adds no synthetic row — a defended call can end all-green at the turn cap, with the declared asserts as the verdict.

## Choosing gates
- **Every voice agent:** `no_spoken_symbols`, `latency`. Add `no_spoken_cardinal_ids` if it reads back any identifier.
- **A line that must be delivered verbatim-in-content** (a disclosure, a total, a confirmation sentence): `spoken_matches_text` — formatting-tolerant, content-strict.
