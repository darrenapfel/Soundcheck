...
- **Sample gallery: a real, unplanted catch.** The gallery gains a third category between "handled" and the deliberately-broken "planted bug" demos: the *well-built* healthcare agent failing on its own. Driven by an adversarial caller it gets talked off the grounded date and confirms a non-Thursday as "this Thursday"; `grounding` + the new `spoken_consistent_with_tool` gate catch it — a live demonstration of a subtle, ship-blocking failure (reproduced 3/3 takes), with a banner stating it is not planted.
