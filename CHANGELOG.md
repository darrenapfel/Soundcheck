...
- **Coding-agent skill + `soundcheck install-skill`.** Ships a progressive-disclosure Agent Skill at `.claude/skills/soundcheck/` (a lean `SKILL.md` entry + `reference/` for commands/gates/scenarios/agents + a `tutorials/zero-to-tuned.md` walkthrough) that teaches a coding agent how to drive Soundcheck, and auto-loads for Claude Code inside the repo. The new `soundcheck install-skill` command installs it into the user-global skills dir — Claude Code (`~/.claude/skills`) by default; `--codex`/`--gemini`/`--all` add the others (Gemini gets a generated `instructions.md` + `skill.yaml`); `--link` symlinks instead of copying. Skill folder added to the package `files` allowlist; `npm run skill:install` alias added.
